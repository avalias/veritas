...
| FW-8 | **POSTPONED (designed 2026-06-11): non-interactive verification — one-shot ZK fault proofs, then validity mode.** The honest path stays exactly as built (per-token roots, ~3% hashing). Rung 1: on fraud, the challenger submits ONE transaction with a succinct proof of "program_root P from agreed root T_i for S_i steps yields X ≠ claimed T_{i+1}" — Sui verifies Groth16 natively (sui::groth16); no bisection, no liveness games; prover = any zkVM running vm::Machine + vm::softfloat unchanged (pure-integer code; minutes–hours per token segment, challenger-only, paid by the slash). Rung 2: bespoke sumcheck/GKR prover — the committed program is uniform layered matmul (sumcheck-optimal); RLC-batching across tokens amortizes the weight stream (the dominant term) once per layer per JUDGMENT vs once per token in decode itself, so prover wall-clock can approach or beat 1× of inference; nonlinears are LUT16-shaped = Lasso/logUp-native; small/binary fields (BabyBear/M31/Binius) make field ops ≈ integer ops; witness commitment = one hash pass ≈ the 3% we already pay. Rung 3: validity mode — proof posted with the assertion (no window), lazy/batched/recursively aggregated. CONSTRAINT: the cheap matmul-sumcheck requires field-linear (integer) semantics — float blocks it (non-associative ⇒ not bilinear). MODEL-PEDIGREE FINDING (researched, sourced in PRIOR_ART): NO publicly downloadable LLM has fully integer-only inference with proven parity as of 2026-06 (I-LLM: right design, no artifacts; BitNet b1.58 2B-4T: official trained-low-bit weights, integer matmul core, but float rescales/softmax/norms; vendor int8 releases: weight-only, float activations). DECISION GATES when resumed: (a) BitNet b1.58 as the integer judge IF it reproduces Microsoft's published benchmarks through our harness after wide-integer rescale/norm/softmax replacement; (b) or official-Qwen-Int8 weights + exact i32/i64 activations gated on published-benchmark reproduction; (c) float-Qwen stays the quality-canonical interactive judge regardless. First experiment on resume: sumcheck one Qwen layer GEMV in BabyBear/M31 batched over 64 positions; measure prover-seconds ÷ inference-seconds. |
| FW-7 | ~~ISA additions for the wide-activation Qwen path~~ **LANDED in v0.4.0**: `LD16` (0x17), `DOT8X16` (0x18, i8 weights × i16 activations), `DOTBM` (0x19, fused dot-line × per-(row,block) i32 multiplier — the Q8_0-style blocked-quantization structure the measured quality campaign forced, one op/block, exact i64 accumulation). All three across the three twins (exec/onestep/Move interp), conformance + trap + fuzz + Move vectors green; the Qwen compiler (`compiler/src/qwen.rs`) and C-14-at-scale use them. STILL OPEN: `AXPY8` line-op (out_line += scalar·in_line) to give row-major V attention a line primitive instead of per-element MAC16s (the native runtime already pays this on the honest path; an opcode would shrink the VM step count). |
| FW-6 | **Float-ISA extension — CHAIN COMPLETE.** Committed-float Qwen = published quality (PPL 34.5974); softfloat fp32 (mul/add/fma/div/sqrt/floor/ftoi/itof/fgt) in Rust AND Move, 8M-case hardware-bitwise fuzz; FDOT (0x1A: committed block dot accumulate) + FOP (0x1B: k-selected scalar float op) across the three twins; float compiler (fqwen.rs) → 32.3M micro-ops/judgment; **C-14-FLOAT GREEN** (hardware-float native == softfloat VM oracle, roots bit-identical); float fraud game + Move conviction of an FDOT step. Earlier core results: Measured: committed-float Qwen3-0.6B = PPL 34.5974 (== published-model bar); tokens + 151,936 logits bit-identical across thread counts AND Metal-GPU vs CPU (fastMathEnabled=false required — wgpu cannot set it, measured 1-ulp divergence; direct Metal exact); committed dot/exp bits golden-pinned cross-platform. Semantics: bf16 weights (exact widening), pinned 16-lane/64-block fma tree (kernels::fkernels — the scalar definition IS the spec), committed polynomial exp (qwen::fmath, no libm anywhere), frozen-artifact rope tables. REMAINING for full FW-6 protocol: float one-step verifier in Move (softfloat, ONE disputed op), float-ISA ops + compiler, committed-Metal full forward. Original sketch: Add `DOT16F`/`DOT16BF` micro-ops whose normative semantics are the SAME canonical binary-tree fp32 reduction that batch-invariant GPU kernels already perform (EigenAI arXiv 2602.00182 §kernels; vLLM/SGLang deterministic modes). Then: the GPU engine is a bit-exact implementation of the committed semantics at ~native speed (no quantization, full model quality), and the one-step verifier checks a single disputed flop via softfloat in pure-integer Move (~150–300 lines per float op — strains Invariant 4, measured before adoption). Must pin: bf16/fp16 storage, fp32 accumulate, exact tree bracketing per reduction size, RN rounding, FTZ/denormal policy, committed polynomial exp/silu (never libm), NaN propagation. Everything else (Merkle pages, bisection, two-level traces, dispute flow) is unchanged — the protocol is arithmetic-agnostic by construction. |
