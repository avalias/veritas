...
| FW-7 | ISA additions for the wide-activation Qwen path (the i32 residual carrier + i16 matmul inputs that real outlier ranges force): `LD16`, a mixed `DOT8X16` (i8 weights × i16 activations), and an `AXPY8` line-op (out_line += scalar·in_line — row-major V attention without per-element MACs). Append-only opcodes; the native runtime already implements these semantics. |
