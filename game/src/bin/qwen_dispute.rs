...
/// Write a self-contained `sui move test` proving the on-chain verifier
/// convicts this exact Qwen fault (verdict 1 = ChallengerWins).
fn emit_move_vector(judge: &JudgeParams, pre_root: &Hash, claimed: &Hash, proof: &StepProof) {
    let sibs = |s: &[Hash]| {
        s.iter().map(|h| format!("x\"{}\"", hexs(h))).collect::<Vec<_>>().join(", ")
    };
    ...
    s.push_str(
        "/// GENERATED by `cargo run -p game --bin qwen_dispute` — a REAL Qwen3-0.6B\n\
         /// fault (one flipped weight bit, isolated by 25 bisection rounds over a\n\
         /// 29.5M-step judgment) convicted by the on-chain one-step verifier.\n\
...
fn main() {
    ...
    // Emit the challenger's atomic-step StepProof as a Move test vector: the
    // ON-CHAIN verifier convicting the REAL Qwen fault. A StepProof is
    // self-contained (opened pages + sibling hashes), so this runs in
    // `sui move test` without the 1 GiB image — the headline trustless-
    // verification artifact.
    let atomic_lo = fault.step;
    let pre_root = challenger.root_at(atomic_lo);
    let claimed_hi = resolver.root_at(atomic_lo + 1); // the resolver's LIE
